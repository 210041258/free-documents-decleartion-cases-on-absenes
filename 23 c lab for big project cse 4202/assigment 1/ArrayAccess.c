...
)
{
    ...
    size_t x,i=(0+1),j=((rowCount)*(colCount)+1),m = ((i+j)/2);
    while(j>i){
        m =((i+j)/2);
        if(((arr+(((rowIndex*colCount)+(colIndex)))))>(arr+(m))){
            i = m + 1 ;
        }
        else{
            j = m - 1 ;
        ...
        if((((arr+(((rowIndex*colCount)+(colIndex)))))==(arr+(i)))){
           return arr+i;
        }
        else{
        return NULL;
